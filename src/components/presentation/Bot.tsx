    ...
    return (
        <>
            <div className={`${isChatbotVisible ? 'z-50' : ''} fixed right-4 bottom-4 w-16 h-16`}>
                ...
                {/* chatbot */}
                <div
                    className={`absolute w-[300px] 2xl:w-[400px] mx-auto bg-white rounded-lg shadow-lg overflow-hidden right-4 bottom-[70px] h-[456px] 2xl:h-[550px] z-40 flex flex-col transition-transform duration-300 ${isChatbotVisible ? 'translate-y-0 opacity-100' : 'translate-y-full opacity-0'}`}
                >
                    {
                        !isEnableChat ? (
                            <div className="bg-[#803DF8] px-4 py-4 text-white relative border-b border-[#FFFFFF3A]">
                                ...
                                <h1 className="text-xl 2xl:text-2xl font-semibold">Hello there <span role="img" aria-label="wave">👋</span></h1>
                                <h2 className="text-xl 2xl:text-2xl font-semibold">How can we help?</h2>
                                <p className='text-[11px] 2xl:text-sm font-normal text-white'>For personalized service, please</p>
                                <button className="mt-2 text-xs 2xl:text-sm text-white font-semibold py-2 px-4 border border-white rounded-full">Log In</button>
                            </div>
                        ) : (
                            <div className="bg-[#803DF8] text-white p-3 border-b border-[#FFFFFF3A] rounded-t-lg flex items-center justify-between">
                                ...
                                <div onClick={toggleCloseChat} className='cursor-pointer'>
                                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
                                        <path d="M18 6L6 18M6 6L18 18" stroke="white" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" />
                                    </svg>
                                </div>
                        ...
                        !isEnableChat ? (
                                ...
                                <h3 className="text-base font-semibold mb-3 text-white">Popular topics</h3>
                                {
                                    qaSet.map((select) => (
                                        <div onClick={() => handleSelectQuestion(select)} key={select?.id} className="bg-white p-3 rounded-lg shadow mb-2 cursor-pointer">
                                            <p className="text-xs 2xl:text-base font-normal">{select.question}</p>
                                        </div>
                                    ))
                                }
                                <p className="text-[10px] 2xl:text-sm text-[#6B6B6B] mt-2">By chatting you accept and agree to AutoFin Credit’s <a href="/user-agreement" className="text-blue-500 underline">User Agreement</a> and AutoFin Credit’s <a href="/frca-policy" className="text-blue-500 underline">Financial and Other Privacy Notices</a>.</p>
                            </div>
                        ) : (
                            <div className="p-4 space-y-4 overflow-y-auto scroll-hidden-functional flex-grow">
                                {/* <div className="flex items-start space-x-2">
                                    ...
                                    <div className="bg-gray-100 p-2 rounded-lg text-sm 2xl:text-base">
                                        <span>👋 Hi there! How can I help?</span>
                                    </div>
                                </div> */}
                                <div className="flex justify-end">
                                    <div className="bg-[#803DF8] text-white p-2 rounded-lg text-sm 2xl:text-base">
                    ...
                    <div className="px-4 py-[6px] 2xl:py-2 border-t border-gray-300 flex items-center">
                        <input type="text" placeholder="Ask anything" className="text-sm 2xl:text-base flex-1 p-2 border border-gray-300 rounded-lg" />
